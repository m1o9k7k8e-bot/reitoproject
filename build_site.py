...
def build():
    ...
    suspicious = conn.execute("""
      SELECT machine_name,page_status,detail_link_count,unique_detail_links,parsed_rows,duplicate_link_count
      FROM collection_issues
      WHERE record_date=?
        AND (page_status != 'ok'
             OR duplicate_link_count > 0
             OR unique_detail_links != parsed_rows)
    ...
    suspicious_rows = "\n".join(
        f"<tr><td>{esc(s[0])}</td><td>{esc(s[1])}</td><td>{s[2]}</td>"
        f"<td>{s[3]}</td><td>{s[4]}</td><td>{s[5]}</td></tr>"
        for s in suspicious
    ) or '<tr><td colspan="6">機種ページ内の解析異常は検出されませんでした</td></tr>'
...
@media(max-width:650px){{.cards{{grid-template-columns:1fr 1fr}} th,td{{font-size:12px;padding:7px}}}}
...
<p class="note">
機種数 {listed_models} ／ 詳細リンク検出 {detail_links}件 ／
ユニーク詳細リンク {unique_detail_links}件 ／ 重複リンク {duplicate_links}件。
v1.2では画面上の「XXXX番台」文字ではなく、各台の「詳細を見る」リンク内の台番号を基準に収集しています。
</p>

<h2>全台一覧</h2>
<p class="note">「当日実績指数」は当日の公開値を相対化したもので、翌日の当たりやすさ・勝率・期待値の予測ではありません。</p>
...
<h2>取得診断</h2>
<table><thead><tr>
<th>機種</th><th>状態</th><th>詳細リンク</th><th>ユニーク</th><th>解析行</th><th>重複</th>
</tr></thead><tbody>{suspicious_rows}</tbody></table>
