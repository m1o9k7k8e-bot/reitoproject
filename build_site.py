...
def build():
    ...
    rows = conn.execute("""
      SELECT machine_number,machine_name,flags,
             big_hits,kakuhen_jitan,max_balls,machine_id
      FROM daily_records
      WHERE record_date=?
    ...
    coverage = conn.execute("""
      SELECT official_units,collected_units,missing_units,
    ...
    if coverage:
        ...
        no_row_models = 0
        detail_links = 0
        unique_detail_links = 0
        duplicate_links = 0

    changes = conn.execute("""
      SELECT change_date,machine_number,
    ...
    all_records = conn.execute("""
      SELECT record_date,machine_number,machine_id,
             machine_name,flags,big_hits,
             kakuhen_jitan,max_balls
      FROM daily_records
    ...
    inactive_dates = {
        ...
        if vals and all(
            ...
            and kakuhen is None
            and balls is None
            for big, kakuhen, balls in vals
        )
    }

    latest_inactive = latest in inactive_dates
    ...
    for mid, items in model_groups.items():
        ...
        for rank, (num, score) in enumerate(
            ordered,
            start=1
        ):
            rank_info[num] = (
    ...
    for r in rows:
        ...
        generation_stats = [
            ...
            if rr[0] not in inactive_dates
        ]

        if r[0] in inactive_dates:
            prev = (
                generation_stats[-1]
                if generation_stats
                else None
        ...
        trend = trend_label(
            [x[7] for x in generation_stats[-3:]]
        )

        rank, total = rank_info.get(
            num,
            (1, 1)
        )
    ...
    table = "\n".join(
        f'<tr data-num="{esc(e["row"][0])}" '
        f'onclick="showHistory(\'{esc(e["row"][0])}\')" '
        f'class="clickrow">'
        f'<td>{esc(e["row"][0])}</td>'
        f'<td>{esc(e["row"][1])}</td>'
        f'<td>{esc(e["row"][2])}</td>'
    ...
    for num, recs in histories.items():
        ...
        last7 = generation_stats[-7:]
        last30 = generation_stats[-30:]

        by_weekday = defaultdict(list)

        for rr in generation_stats:
            by_weekday[
                weekday_jp(rr[0])
            ].append(rr)

        if latest_rec[0] in inactive_dates:
            prev = (
                generation_stats[-1]
                if generation_stats
                else None
        ...
        today_wd_stats = weekday_stats.get(
            latest_weekday
        )
        ...
        if latest_inactive:
            ...
            today_wd_stats
            and today_wd_stats["count"] >= 2
        ):
            today_wd_note = (
                f'{latest_weekday}曜 '
                f'{today_wd_stats["count"]}日平均: '
                f'大当り '
                f'{today_wd_stats["big_hits"]} / '
                f'最大持玉 '
                f'{today_wd_stats["max_balls"]}'
            )
    ...
    if latest_inactive:
        inactive_note = (
            f'<p class="note warnbox">'
            f'最新記録日 {esc(latest)} は'
            f'「休業／未更新候補」です。'
            f'全台の実績値が空欄のため、'
            f'平均・曜日傾向・直近トレンドの'
            f'計算から除外しています。'
...
機種数 {listed_models}
／ 詳細リンク検出 {detail_links}件
／ ユニーク詳細リンク {unique_detail_links}件
／ 重複リンク {duplicate_links}件。
  ...
  const recRows =
    [...h.records]
      ...
      .map(
        ...
        ${{
          r.inactive
          ? '<span class="pill inactive-pill">休業／未更新候補</span>'
          : ''
        }}
